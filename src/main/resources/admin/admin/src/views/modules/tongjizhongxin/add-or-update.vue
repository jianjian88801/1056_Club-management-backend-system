    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="统计类型" prop="tongjileixing">
          <el-select v-model="ruleForm.tongjileixing" placeholder="请选择统计类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="统计数量" prop="tongjishuliang">
          <el-input v-model="ruleForm.tongjishuliang" 
              placeholder="统计数量" clearable  :readonly="ro.tongjishuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="统计数量" prop="tongjishuliang">
              <el-input v-model="ruleForm.tongjishuliang" 
                placeholder="统计数量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tongjileixing = true;
          ...
          if(o=='tongjishuliang'){
            this.ruleForm.tongjishuliang = obj[o];
	    this.ro.tongjishuliang = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tongjizhongxin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tongjizhongxinCrossAddOrUpdateFlag = false;
                  this.parent.search();
